...
if __name__ == "__main__":
    ...
    while len(target_frames) < 2*args.num_segments:
        ...
        latents = []
        coefficients = torch.rand(len(frames))
                ...
                for k in range(num_batches):
                    ...
                    for index in indices[k*minibatch:k*minibatch+minibatch]:
                        ...
                        #print(f"k: {k}, interpolated_point: {interpolated_point}")
                            #print(f"Indexing into coefficient {index}")
                        interpolated_latent = coefficients[index] * delta + latent_1
    ...
    # create and save frames TODO(vmreyes): use the latent coefficient
    frame_num = 0
    final_frames = []
    for j in range(0, len(target_frames), 2):
        latent_1_index = target_frames[j]
        ...
        for k in range(num_frames):
            interpolated_point = latents[k+latent_1_index]
    ...
    with open(args.out_dir + "lpips" + latent_description + ".data", 'wb') as filehandle:
        pickle.dump({"lpips":p_losses, "latents":latent_path, "target_frames":target_frames}, filehandle)
    fig, ax = plt.subplots()
    ax.plot(p_losses)
